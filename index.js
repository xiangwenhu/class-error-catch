...
const { getOwnPropertyDescriptors, defineProperty, getPrototypeOf } = Object;
 ...
 */
function defaultFilter(propertyName, descriptor) {
    return descriptor.configurable && !WHITE_LIST.includes(propertyName);
}
...
module.exports = function createCatchError({
    ...
    function catchClass(targetArg, ...params) {
        ...
        for (let [property, descriptor] of Object.entries(descriptors)) {
            if (
                defaultFilter(property, descriptor) &&
                (!filter ||
                    (filter &&
